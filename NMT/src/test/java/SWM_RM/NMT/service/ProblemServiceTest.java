...
public class ProblemServiceTest {
    ...
    public void problemListFilterService(){
        ...
        List<ProblemListDTO> findProblemListFilter = problemService.problemListFilterService(
                "요약",
                "고려대학교",
                2022L);
        System.out.println("---------세 필터 적용-----------");
        for(ProblemListDTO p : findProblemListFilter){
            System.out.println(p.getProbTitle()+" "+p.getUniversity().getUniversityName()+
                    " "+p.getProbType().getTypeName());
        }

        List<ProblemListDTO> findProblemListFilter1 = problemService.problemListFilterService(
                "요약",null,null
        );
        System.out.println("---------문제유형(요약) 필터 적용-----------");
        for(ProblemListDTO p : findProblemListFilter1){
            System.out.println(p.getProbTitle()+" "+p.getUniversity().getUniversityName()+
                    " "+p.getProbType().getTypeName());
        }

        List<ProblemListDTO> findProblemListFilter2 = problemService.problemListFilterService(
                null,"인하대학교",null
        );
        System.out.println("---------대학이름(인하대학교) 필터 적용-----------");
        for(ProblemListDTO p : findProblemListFilter2){
            System.out.println(p.getProbTitle()+" "+p.getUniversity().getUniversityName()+
                    " "+p.getProbType().getTypeName());
        }

        List<ProblemListDTO> findProblemListFilter3 = problemService.problemListFilterService(
                null,null,2022L
        );
        System.out.println("---------출제년도 필터 적용-----------");
        for(ProblemListDTO p : findProblemListFilter3){
